[mt,failrate,c,mt1,failrate1,c1,mt2,failrate2,c2,B,M,cloud,B1,M1,cloud1,B2,M2,cloud2] = mian(40,200);
function [mt,failrate,c,mt1,failrate1,c1,mt2,failrate2,c2,B,M,cloud,B1,M1,cloud1,B2,M2,cloud2] = mian(m,n)
    for i = m : 40 : n
        ...
        [mt((i-m)/40+1),failrate((i-m)/40+1),c((i-m)/40+1)] = result(B,M,cloud);
        [mt1((i-m)/40+1),failrate1((i-m)/40+1),c1((i-m)/40+1)] = result(B1,M1,cloud1);
        [mt2((i-m)/40+1),failrate2((i-m)/40+1),c2((i-m)/40+1)] = result(B2,M2,cloud2);
    end
end

function [mt,failrate,c] = result(B,M,cloud)
    c=0;
    fail=0;
    w=0;
    for i = 1:size(B,2)
...
function [B,M,cloud]=Algorithm2(B,M,i,j,cloud)   %Schedule the j-th task of the i-th application
    ...
    elseif j==0                   %For dummy task start tasks, scheduling is performed on the vehicle
        ...
        st = -inf;
        t=subdeadline(B,i,j);      %The children task is a virtual task   
        x=findcarpos(B,i,t);      
        suc=findsuc(B,i,j);
        if suc==size(B(i).nodes,1)-1  
            E1=[];
        else
            E1=unique(findsucpos(B,i,j));  
        end
...
function [B,M] = schedule1(i,j,t,B,M,cloud)
    ...
    elseif j>=1 && j <= (size(B(i).nodes,1)-2)   %%real task
        ...
        ft=finishtime(i,j,0,t,B,M,cloud);
        if ft <= B(i).req
            B(i).nodes(j+1).st=ft;
            B(i).nodes(j+1).ft=ft;
            B(i).nodes(j+1).where=0;
...
function [B,M,cloud]=cDCDS(i,B,M,cloud)
    ...
    for j = pre
        ...
        if flag == 0 || round(est1 + trans * (80 *10^-9 + 7.81 * 10^-9),3) > B(i).req || B(i).nodes(j+1).statue==0
            Q=[i,j;Q];
    ...
    while ~isempty(Q)
        ...
        if j == size(B(i).nodes,1)-1 || j == 0
            ...
            for pre1=pre
                ...
                if round(est2 ,3) + B(i).nodes(pre1+1).rtc > B(i).req || B(i).nodes(pre1+1).statue == 0
                    Q = [i,pre1;Q];
